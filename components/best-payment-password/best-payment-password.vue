<template>
	<view class="best-modal digital-keyboard-modal" :class="{'best-modal-active':_show}">
		...
		<view style=" position: fixed; top: 15%; left: 11%; width: 78%; height: 440upx; background: white; z-index: 2; border-radius: 8upx; ">
			<view style=" height: 100upx; line-height: 100upx; text-align: center; border-bottom: 1px solid #EEE; color: #999; font-size: 30upx; ">
				请输入支付密码
			</view>
			<view class="">
				<view style=" height: 30px; line-height: 40px; text-align: center; color: #5A5A5AFF; ">
					提现
				</view>
				<view style=" font-size: 28px; text-align: center; line-height: 40px; height: 40px; ">
					￥{{Fmoney}}
				</view>
				<view style=" text-align: center; height: 40px; line-height: 40px; color: rgb(153, 153, 153); ">
					需扣除手续费(￥3.00)及税费(￥{{Fmsg}})
				</view>
				<view class="pwd-box clearfix" @click="getKeyboard">
					<view class="pwd-text" v-for="(item,index) in _digits" :key="index" :class="{borleft:(index==0), active:(activeInput==index)} ">{{payPassWord[index]}}</view>
				</view> 
			</view>
	...
	export default {
		...
		methods: {
			initData: function() {
				...
				this.Fmoney = this.money;
				this.Fmsg = this.money*0.07;
				this.Fmsg=this.Fmsg.toFixed(2);
			},
			forgetPwd: function() {
				console.log('跳转到忘记密码')
			},
