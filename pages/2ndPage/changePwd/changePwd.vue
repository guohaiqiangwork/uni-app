	...
	<view>
		...
		<view class="form_body">
			<view class="form_list">
				...
				<view class="list_right">
					 <input class="list_input" @input="keynPwd" password  maxlength="20" placeholder="新密码" />
			...
			<view class="form_list">
				...
				<view class="list_right">
					 <input class="list_input"  @input="keyqPwd" password  maxlength="20" placeholder="确认密码" />
				</view>
			</view>
		</view>
...
                        that.clear = setInterval(that.countDown,1000);
			...
			this.$http.get('/api/v1/user_modify', {phone: this.keyPhoneS,code:this.keyYzmS,messageid:this.messageid,password:this.keyqPwds}).then(res => {
					uni.showToast({
					title: res.data.message,
					icon:'none',
					duration: 2000,
