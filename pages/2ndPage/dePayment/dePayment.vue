...
<!-- 		<view class="bottom_only" v-if="orderlist.status==2">
			...
			<!-- <view style="height: 200upx;" ></view> -->
			 <view class="bottom_add"  v-if="orderlist.status==2">
				<view class="fixed_btm" @click="ljzf('2')">
					确认收货
				</view>
			</view>
		<!-- 已完成 -->
...
<script>
	export default {
		...
		onLoad(option){
			var that=this;
			this.$http.get('/api/v1/selectOrderMessage', {userid: uni.getStorageSync('id'),id:option.id,userid:option.userid}).then(res => {	
		...
		methods: {
			ljzf(index){
					if(index=='1'){//待付款
						uni.navigateTo({
							url:"../payCashi/payCashi?id="+this.orderlist.id+"&paymoney="+this.paymoney+""
						})
					}else{
						//var data=this.orderlist[index];
						var id= uni.getStorageSync('id')
						var that=this;
						this.$http.get('/api/v1/payOrder', {userid: id,id:that.orderlist.id,cid:that.orderlist.cid}).then(res => {
