                    ...
                    <view class="rece_card" v-for="(item, index) in listData" :key="item.value" v-if="listData!=''">
                    	...
                    	<view class="card_top">
							...
							<view class="top_right">
								<text class="tr_text" style="width: 100%;">{{item.status=='0'?'审核中':item.status=='1'?'审核成功':'审核失败'}}</text>	
								<!-- <view class="tr_icon">
...
<script>
	...
	export default {
		...
		data() {
			return {
				TabCur:0,
				tabList: [{ name: '审核中' }, { name: '申请成功' },{ name: '申请失败' },],
				winheight:'1224upx',
				listData:"",
			}
		},
		...
		methods: {
			showlistdata(){
				...
				this.$http.get('/api/v1/getTransfer', {userid: id,status:this.TabCur}).then(res => { 
