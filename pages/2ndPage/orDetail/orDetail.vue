<template>
	...
	<view class="bottom_button" v-if="orinforlist.status==2">
		<view class="bottombg">
				...
				<view class="bgjin" @click="qrsh">确认收货</view>
			</view>
		</view>
	</view>
</template>
		...
		data() {
			...
			methods:{
				qrsh(){
					...
					// 	mask:true,
					// });
					this.$http.get('/api/v1/payIntegralOrder', {userid: uni.getStorageSync('id'),id:this.chanpinId}).then(res => {
						uni.showToast({
				...
				ljzf(){
					...
					this.$http.get('/api/v1/payIntegralOrder', {userid: uni.getStorageSync('id'),id:this.chanpinId,tradenum:this.chanpinNum,addressid:this.addressPost.id}).then(res => {
						uni.showToast({
							title: res.data.message,
							icon:'none',
							duration: 2000,
						...
						if(res.data.resultCode==200){
							uni.reLaunch({
								url: '../orDetail2/orDetail2?chanpinId='+this.chanpinId//向新增地址页传递个标识参数
							});
						}else if(res.data.resultCode==400){
							//有误
						}
				...
				TimeDown(endDateStr) {
								...
								var that=this;
							    //结束时间
							    var endDate = new Date(endDateStr.replace(new RegExp(/-/gm) ,"/"));
							    //当前时间
